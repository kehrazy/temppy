...
template <char... Cs> struct String {
  ...
  template <size_t Index> static constexpr char at() {
...
template <char... Cs> constexpr char String<Cs...>::data[];

using EmptyString = String<>;

template <typename S1, typename S2> struct StringConcat;
...
template <typename S, char C> struct StringAppend;

template <char... Cs, char C> struct StringAppend<String<Cs...>, C> {
...
template <auto S, typename Seq> struct ToString;
...
template <auto S>
using ToString_t =
    typename ToString<S, std::make_index_sequence<S.size - 1>>::type;

} // namespace temppy

template <temppy::fixed_string S> consteval auto operator""_fs() { return S; }

template <temppy::fixed_string S> consteval auto operator""_s() {
  return temppy::ToString_t<S>{};
}
